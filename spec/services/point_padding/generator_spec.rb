...
RSpec.describe PointPadding::Generator do
  ...
  let(:source_point) do
    create(:point, user: user, lonlat: 'POINT(13.4050 52.5200)', timestamp: 1_700_000_000, tracker_id: 'demo')
  end
  ...
  describe '#build_points' do
    context 'with valid params (after, 5 points over 1 hour)' do
      ...
      it 'inherits tracker_id from the source point so padded points render in the same route bucket' do
        expect(points.map(&:tracker_id)).to all(eq('demo'))
      end
    end
